  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const isRateLimit = msg.includes('rate limit') || msg.includes('rate-limit') ||
                          msg.includes('request limit') || code === -32005;
      const isGasTip    = msg.includes('tip cap') || msg.includes('gas price') ||
                          code === -32603;

      // Retry on rate-limit or gas tip errors (up to max retries)
      if ((isRateLimit || isGasTip) && attempt < retries) {
        const delay = attempt * 1500; // 1.5 s, then 3 s
        showToast('warning', `Retrying (${attempt}/${retries - 1})`,
          isRateLimit ? `RPC rate limited — retrying in ${delay / 1000}s...`
                      : `Gas error — retrying with higher tip in ${delay / 1000}s...`);
  ...
  try {
    ...
    if (err.code === 4902) {
      await window.ethereum.request({
        ...
        params: [{ chainId: '0x13882', chainName: 'Polygon Amoy Testnet', rpcUrls: [
            'https://polygon-amoy-bor-rpc.publicnode.com',
            'https://rpc.ankr.com/polygon_amoy',
            'https://rpc-amoy.polygon.technology/'
          ], nativeCurrency: { name: 'MATIC', symbol: 'MATIC', decimals: 18 }, blockExplorerUrls: ['https://amoy.polygonscan.com/'] }]
      });
    } else { throw err; }
  }
}
...
async function createRole() {
  ...
  try {
    ...
    const tx = await sendTx(() => contract.createRole(name, AMOY_GAS));
...
async function createPermission() {
  ...
  try {
    ...
    const tx = await sendTx(() => contract.createPermission(name, AMOY_GAS));
    showToast('info', 'Awaiting Confirmation', `Tx: ${shortAddr(tx.hash)}`);
    await tx.wait();
...
async function assignRole() {
  ...
  try {
    ...
    const tx = await sendTx(() => contract.assignRole(addr, BigInt(roleId), AMOY_GAS));
    showToast('info', 'Awaiting Confirmation', `Tx: ${shortAddr(tx.hash)}`);
    await tx.wait();
    incStaffCount();
...
async function suspendUser() {
  ...
  try {
    ...
    const tx = await sendTx(() => contract.suspendUser(addr, AMOY_GAS));
    await tx.wait();
...
async function activateUser() {
  ...
  try {
    ...
    const tx = await sendTx(() => contract.activateUser(addr, AMOY_GAS));
    await tx.wait();
...
async function revokeUser() {
  ...
  try {
    ...
    const tx = await sendTx(() => contract.revokeUser(addr, AMOY_GAS));
    await tx.wait();
